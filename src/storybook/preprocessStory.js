...
export default async function preprocessStory (rawStory, {cache}) {
  ...
  if(rawStory.GRID.MOBILE_M.active) {
    ...
    story.grids.MOBILE_M = `grid: ${rawStory.GRID.MOBILE_M.content}; grid-gap: ${rawStory.GRID_GAP.MOBILE_M}px; ${flexedClasses}`
  }

  if(rawStory.GRID.MOBILE_L.active) {
    const flexedClasses = flexClasses(rawStory.GRID.MOBILE_L.components)
    story.grids.MOBILE_L = `grid: ${rawStory.GRID.MOBILE_L.content}; grid-gap: ${rawStory.GRID_GAP.MOBILE_L}px; ${flexedClasses}`
  }

  if(rawStory.GRID.TABLET.active) {
    const flexedClasses = flexClasses(rawStory.GRID.TABLET.components)
    story.grids.TABLET = `grid: ${rawStory.GRID.TABLET.content}; grid-gap: ${rawStory.GRID_GAP.TABLET}px; ${flexedClasses}`
  }

  if(rawStory.GRID.LAPTOP.active) {
    const flexedClasses = flexClasses(rawStory.GRID.LAPTOP.components)
    story.grids.LAPTOP = `grid: ${rawStory.GRID.LAPTOP.content}; grid-gap: ${rawStory.GRID_GAP.LAPTOP}px; ${flexedClasses}`
  }

  if(rawStory.GRID.LAPTOP_L.active) {
    const flexedClasses = flexClasses(rawStory.GRID.LAPTOP_L.components)
    story.grids.LAPTOP_L = `grid: ${rawStory.GRID.LAPTOP_L.content}; grid-gap: ${rawStory.GRID_GAP.LAPTOP_L}px; ${flexedClasses}`
  }

  if(rawStory.GRID.LAPTOP_XL.active) {
    const flexedClasses = flexClasses(rawStory.GRID.LAPTOP_XL.components)
    story.grids.LAPTOP_XL = `grid: ${rawStory.GRID.LAPTOP_XL.content}; grid-gap: ${rawStory.GRID_GAP.LAPTOP_XL}px; ${flexedClasses}`
  }
